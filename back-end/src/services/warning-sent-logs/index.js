...
export const f_create_warning_sent_log = async ({
  ...
  return await prisma.warning_logs_sent.create({
    data: {
      user_id: user_id,
      warningId: warningId,
      channel: channel,
...
export const f_delete_warning_log = async ({ id }) => {
  ...
  return await prisma.warning_logs_sent.delete({
    where: {
      id: id,
    },
  });
...
export const f_update_warning_log = async ({ id, item_info }) => {
  ...
  return await prisma.warning_logs_sent.update({
    where: {
      id: id,
    },
    data: item_info,
...
export const f_get_by_id_warning_log = async ({ id }) => {
  ...
  return await prisma.warning_logs_sent.findUnique({
    where: {
      id: id,
    },
  });
...
export const f_get_all_warning_sent_logs = async () => {
  return await prisma.warning_logs_sent.findMany();
};
